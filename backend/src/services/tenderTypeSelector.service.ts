 ...
 */
export async function getTenderTypeByCode(
  code: string,
): Promise<TenderTypeDefinition> {
...
export async function suggestTenderType(
  ...
  if (input.isEmergency || input.isSingleSource) {
    if (input.procurementType === "goods") {
      ...
      suggestions.push({
        ...
        reasons: [
          input.isEmergency
            ? "Emergency procurement requirement"
      ...
      suggestions.push({
        ...
        reasons: [
          input.isEmergency
            ...
            : "Single source (only one provider)",
          "Direct procurement applicable",
          "Justification required",
        ],
  ...
  const candidateTypes = allTypes.rows
    .filter((type: TenderTypeDefinition) => {
      ...
      const meetsMax =
        type.max_value_bdt === null ||
        input.estimatedValue <= type.max_value_bdt;
...
function scoreTenderType(type: TenderTypeDefinition): TenderTypeSuggestion {
  ...
  const formattedMax = type.max_value_bdt
    ? formatBDT(type.max_value_bdt)
    : "Unlimited";
...
export async function getValueRangesForProcurementType(
  ...
  const normalTypes = types.filter(
    (t) =>
      !t.is_direct_procurement &&
      !t.is_international &&
      !["PG4", "PG5A", "PPS2"].includes(t.code),
  );
...
function buildRangeLabel(type: TenderTypeDefinition): string {
  ...
  const maxFormatted = type.max_value_bdt
    ? formatBDT(type.max_value_bdt)
    : "Unlimited";
