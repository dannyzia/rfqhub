/**
 * tenderTypeSelector.service.test.ts
 *
 * The service is a PROTECTED FILE (extend-only).  All tests mock the database
 * pool so they never hit a real PostgreSQL instance.  Redis is already mocked
 * to return null for .get() when NODE_ENV === 'test' (see config/redis.ts),
 * so the service always falls through to the mocked DB.
 */
...
describe("getValueRangesForProcurementType", () => {
  ...
  test("should return value ranges for goods procurement", async () => {
    const result =
      await tenderTypeService.getValueRangesForProcurementType("goods");
  ...
  test("should return value ranges for works procurement", async () => {
    const result =
      await tenderTypeService.getValueRangesForProcurementType("works");
  ...
  test("should return correct special cases for services", async () => {
    const result =
      await tenderTypeService.getValueRangesForProcurementType("services");
  ...
  test("should format human-readable labels", async () => {
    ...
    expect(
      result.ranges.some(
        (r) => r.label.includes("Lac") || r.label.includes("Crore"),
      ),
    ).toBe(true);
  });
...
describe("getTenderTypeByCode", () => {
  ...
  test("should return PG2 details", async () => {
    ...
    expect(Number(type.min_value_bdt)).toBe(800001);
