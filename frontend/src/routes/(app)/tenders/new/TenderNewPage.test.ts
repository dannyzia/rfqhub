...
describe("Tender Creation Form - Cascading Dropdowns", () => {
  ...
  describe("Procurement Type Selection", () => {
    ...
    test("should show outsourcing checkbox for services", async () => {
      ...
      const procurementSelect = screen.getByLabelText(/procurement type/i);
      await fireEvent.change(procurementSelect, {
        target: { value: "services" },
      });

      await waitFor(() => {
        expect(
          screen.getByText(/outsourcing service personnel/i),
        ).toBeInTheDocument();
        expect(
          screen.queryByText(/international bidding/i),
  ...
  describe("Estimated Cost Dropdown", () => {
    ...
    test("should show loading state", async () => {
      ...
      // Use specific text match to avoid multiple "loading" matches in DOM
      expect(screen.getByText("Loading value ranges...")).toBeInTheDocument();
    });
  });
        ...
        ranges: [
          {
            ...
            minValue: 0,
            maxValue: null,
            suggestedTypes: ["PPS2", "PPS3"],
          },
        ],
        specialCases: {
          outsourcingPersonnel: { available: true, type: "PPS2" },
        },
      };
  ...
  describe("Tender Type Filtering", () => {
    test("should render cost options in a select dropdown", async () => {
      ...
      await waitFor(() => {
        const costSelect = screen.getByLabelText(
          /estimated cost/i,
    ...
    test("should show tender type dropdown when procurement type selected", async () => {
      ...
      await waitFor(() => {
        const tenderTypeSelect = screen.getByLabelText(
          /tender type/i,
        ) as HTMLSelectElement;
  ...
  describe("Error Handling", () => {
    ...
    test("should require form validation before submission", async () => {
      ...
      const procurementSelect = screen.getByLabelText(
        /procurement type/i,
      ) as HTMLSelectElement;
